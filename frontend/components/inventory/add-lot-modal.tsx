...
export function AddLotModal({ isOpen, onClose, onSuccess, centerId }: AddLotModalProps) {
    ...
    useEffect(() => {
        ...
        if (isOpen) {
            const loadVaccineTypes = async () => {
                // Asumiendo que tienes un endpoint para obtener los tipos de vacuna
                const { data } = await fetchVaccineTypes('/other/vaccines');
                if (data) {
    ...
    const onSubmit = async (values: z.infer<typeof lotSchema>) => {
        ...
        const { error } = await submitLot('/inventory/lots', {
            method: 'POST',
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Complete los detalles del lote que ha llegado al centro de vacunación.
                    </DialogDescription>
                </DialogHeader>
                <Form {...form}>
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    ...
                                    <FormControl><Input placeholder="Ej: PAA165689" {...field} /></FormControl>
                                    <FormMessage />
                                </FormItem>
                            )}
                        />
